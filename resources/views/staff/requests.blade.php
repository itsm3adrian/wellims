        ...
        <div class="body-wrapper">
            ...
            <div class="container-fluid">
                                ...
                                <div class="table-responsive">
                                    <table class="table text-nowrap mb-0 align-middle">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td><span class="fw-normal">{{$request->sfname}}</span></td>
                                                <td><span class="fw-normal">{{$request->semail}}</span></td>
                                                <td><span class="fw-normal">{{$request->sdept}}</span></td>
                                                <td><span class="fw-normal">{{$request->srollno}}</span></td>
                                                <td><span class="fw-normal">{{$request->product->product_title}}</span>
                                                </td>
                                                <td><span
                                                        class="fw-normal">{{$request->equipmentProduct->product_title}}</span>
                                                </td>
                                                <td>
                                                    ...
                                                    <button class="btn btn-primary accept-btn"
                                                        data-request-id="{{$request->id}}">Accept</button>
                                                    @endif
                                                </td>
                                                <td>
                                                    @if ($request->req_status == 2)
                                                    <button class="btn btn-danger deny-btn" disabled>Denied</button>
                                                    @else
                                                    <button class="btn btn-danger deny-btn"
                                                        data-request-id="{{$request->id}}">Deny</button>
                                                    @endif
                                                </td>
                                            </tr>
                                            @endforeach
                ...
                <div class="py-6 px-6 text-center">
                    <p class="mb-0 fs-4">Design and Developed by Vishal Ingale</p>
                </div>
            </div>
        </div>
    </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        acceptButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                var staffName = '{{ session("staff")["fullname"] }}';
                // Update button color and text immediately
                this.classList.remove('btn-primary');
                this.classList.add('btn-success');
                this.textContent = 'Accepted';

                var correspondingDenyButton = document.querySelector(
                    '.deny-btn[data-request-id="' + requestId + '"]');
                correspondingDenyButton.disabled = true;


                // AJAX request to update status in the backend
                fetch('/changestatus/' + requestId, {
                        method: 'POST',
                        headers: {
                        ...
                        body: JSON
                            .stringify({
                                staffName: staffName
                            }) // You can add data to send to the server if needed
                    })
                    .then(response => {
                        if (!response.ok) {
        ...
        denyButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                ...
                var correspondingAcceptButton = document.querySelector(
                    '.accept-btn[data-request-id="' + requestId + '"]');
                correspondingAcceptButton.disabled = true;

                fetch('/changestatusdeny/' + requestId, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'

                    ...
                    <ul id="sidebarnav">
                        ...
                        <li class="sidebar-item">
                            <a class="sidebar-link" href="" aria-expanded="false">
                                ...
                                <span class="hide-menu">Staff Requests</span>
                            </a>
                        </li>
                            ...
                            <li class="nav-item dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-end dropdown-menu-animate-up"
                                    ...
                                    <div class="message-body">
                                        ...
                                        <a href="{{route('admin.logout',$admin->id)}}"
                    ...
                    <div class="col-lg-8 d-flex align-items-stretch">
                        <div class="card w-100">
                            <div class="card-body p-4">
                                <div class="row">
                                    <div class="col-md-9">
                                        <h5 class="card-title fw-semibold mb-4">Staff Requests</h5>
                                    </div>
                                    ...
                                    <table class="table text-nowrap mb-0 align-middle">
                                        <thead class="text-dark fs-4">
                                            <tr>
                                                ...
                                                <th class="border-bottom-0">
                                                    <h6 class="fw-semibold mb-0">Accepted by Staff</h6>
                                                </th>
                                                <th class="border-bottom-0">
                                                    <h6 class="fw-semibold mb-0">Development Board</h6>
                                                </th>
                                                <th class="border-bottom-0">
                                                    <h6 class="fw-semibold mb-0">Equipment</h6>
                                                </th>
                                                <th class="border-bottom-0">
                                                    <h6 class="fw-semibold mb-0">Student Email</h6>
                                                </th>
                                                <th class="border-bottom-0">
                                                    <h6 class="fw-semibold mb-0">Student Department</h6>
                                                </th>
                                                <th class="border-bottom-0">
                                                    <h6 class="fw-semibold mb-0">Student Roll Number</h6>
                                                </th>
                                                <th class="border-bottom-0">
                                        ...
                                        <tbody>
                                            @foreach($staffreqs as $staffreq)
                                            <tr>
                                                <td><span class="fw-normal">{{$staffreq->sfname}}</span></td>
                                                <td><span class="fw-normal">{{$staffreq->bystaff}}</span></td>
                                                <td><span class="fw-normal">{{$staffreq->product->product_title}}</span>
                                                </td>
                                                <td><span
                                                        class="fw-normal">{{$staffreq->equipmentProduct->product_title}}</span>
                                                </td>

                                                <td><span class="fw-normal">{{$staffreq->semail}}</span></td>
                                                <td><span class="fw-normal">{{$staffreq->sdept}}</span></td>

                                                <td><span class="fw-normal">{{$staffreq->srollno}}</span></td>
                                                <td>
                                                    @if ($staffreq->adminreq_status == 1)
                                                    <button class="btn btn-success" disabled>Approved</button>
                                                    @else
                                                    <button class="btn btn-primary approve-btn"
                                                        data-staffreq-id="{{$staffreq->id}}">Approve</button>
                                                    @endif
                                                </td>
                                                <td>
                                                    @if ($staffreq->adminreq_status == 1)
                                                    <button class="btn btn-danger reject-btn" disabled>Rejecetd</button>
                                                    @else
                                                    <button class="btn btn-danger reject-btn"
                                                        data-staffreq-id="{{$staffreq->id}}">Reject</button>
                                                    @endif
                                                </td>
                                            </tr>
                                            @endforeach
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        approveButtons.forEach(function(button) {
            button.addEventListener('click', function() {
                var requestId = this.getAttribute('data-staffreq-id');

                // Update button color and text immediately
                this.classList.remove('btn-primary');
                this.classList.add('btn-success');
                this.textContent = 'Approved';

                var correspondingRejectButton = document.querySelector(
                    '.reject-btn[data-staffreq-id="' + requestId + '"]');
                correspondingRejectButton.disabled = true;


                // AJAX request to update status in the backend
                fetch('/changeadminreqstatus/' + requestId, {
                        method: 'POST',
                        headers: {
                        ...
                        body: JSON
                            .stringify({}) // You can add data to send to the server if needed
                    })
                    .then(response => {
                        if (!response.ok) {
                        ...
                        // For example, if the server returns JSON data, you can parse it and use it
                        return response.json();
                    })
                    .then(data => {
                        // Optional: Handle data returned from the server
                        console.log(data);
                    })
                    .catch(error => {
